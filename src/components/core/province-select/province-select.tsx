...
export default function ProvinceSelect() {
  ...
  return (
    <Select onValueChange={(val) => setSelectedProvince(val)} value={selectedProvince}>
      <SelectTrigger className="max-w-[180px] flex items-center gap-2 bg-[var(--color-input-bg)] px-3 py-2 rounded-md">
        <MapPin className="w-5 h-5 text-[var(--color-primary)] flex-shrink-0" />
        <span className="truncate">{province?.name || 'Todas las provincias'}</span>
      </SelectTrigger>
